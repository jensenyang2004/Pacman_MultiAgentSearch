...
class ExpectimaxAgent(MultiAgentSearchAgent):
  ...
  def Expectimax(self, currentGameState: GameState, depth, agentIndex):
    ...
    if(agentIndex == 0):
      for oneAction in actions:
        successorGameState = currentGameState.generateSuccessor(agentIndex, oneAction)
        score = self.Expectimax(successorGameState, depth - 1, agentIndex + 1)
        minimax = max(score, minimax)
    else:
      
      for oneAction in actions:
        successorGameState = currentGameState.generateSuccessor(agentIndex, oneAction)
        minimax += self.Expectimax(successorGameState, depth - 1, (agentIndex + 1)%NumAgents)
      minimax /= len(actions)
    return minimax
  ...
  def getAction(self, gameState: GameState):
    ...
    for action in actions:
      successor = gameState.generateSuccessor(0, action)
      score = self.Expectimax(successor, trueDepth - 1, 1)
      if score > valuation: 
        valuation = score
        finalAction = action
    return finalAction
...
def betterEvaluationFunction(currentGameState: GameState):
  ...
  currentScore, distanceToFood, distanceToGhost, capsulesNum, foodLeft = currentGameState.getScore(), 1e308, 1e308, len(capsule), len(foods)

  score = 0
  for ghost in ghosts:
    distanceToGhost = min(distanceToGhost, manhattanDistance(Pos, ghost.getPosition()) - ghost.scaredTimer)

  for food in foods:
    distanceToFood = min(distanceToFood, manhattanDistance(Pos, food))

  if distanceToFood == 1e308:
    distanceToFood = 0
